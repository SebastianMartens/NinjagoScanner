...
- **PictureService** (`picture_service/`, Python) — manages card photos and their sidecars: runs Gemini-based AI analysis via `langchain-google-genai` (orchestrated in `card_analysis.py`, with the Gemini calls in `card_analysis_stage_1_and_2.py` and deterministic catalog matching in `card_analysis_stage_3.py`), reads/writes photo bytes in an S3 bucket keyed by a generated photo ID (`photo_store.py`, via `aioboto3`), reads/writes sidecar records in a DynamoDB table via `sidecar_store.py` (the repository entry point, composing raw DynamoDB access in `sidecar_table.py` with an in-memory write-through cache in `sidecar_cache.py`), and applies manual sidecar edits. Consults CatalogService via its own gRPC client (`catalog_client.py`) for series/card matching — never reads `cardInfos` locally. Exposes `CardPictureService` gRPC service (`picture_scanner_service.py`): `Scan`, `UploadPhoto` (client-streaming; assigns the photo ID, stores the bytes, triggers AI analysis), `GetPhotoDownloadUrl` (short-lived pre-signed S3 GET URL), `ListCards`, `UpdateSidecar`/`UpdateSetName`/`UpdateCardNumber`/`UpdateCardLanguage`/`UpdateReviewStatus`, `MigrateSidecars`, `DeletePhoto`. The only service that ever holds AWS credentials.
...
Internal modules/classes in PictureService (e.g. `photo_store.py`, `sidecar_store.py` and its collaborators `sidecar_table.py`/`sidecar_cache.py`) aren't exposed as a public API — Python has no `InternalsVisibleTo` equivalent, so this is convention (leading-underscore submodules where relevant) rather than an enforced boundary.
