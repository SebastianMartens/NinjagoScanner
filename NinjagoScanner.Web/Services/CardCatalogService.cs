...
/// <summary>
/// Merges catalog data (from CatalogService) with scanned photo data (from PictureService) into
/// the shapes the app's Razor pages render. All card photo/sidecar access is delegated to
/// PictureService via gRPC, keyed by generated photo ID rather than file name; this class never
/// holds AWS credentials itself — photo bytes and download URLs both come from PictureService.
/// </summary>
    ...
    long maxUploadBytes)
{
    ...
    public async Task<IReadOnlyList<GalleryCardItem>> GetGalleryCardsAsync(string series, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var (card, matchedPhoto, photoCount) in matchedCards)
        {
            result.Add(new GalleryCardItem
            {
                ...
                ImageUrl = matchedPhoto is null ? null : await GetDownloadUrlAsync(matchedPhoto.PhotoId, cancellationToken),
    ...
    private async Task<string> GetDownloadUrlAsync(string photoId, CancellationToken cancellationToken)
    {
        using var channel = GrpcChannel.ForAddress(pictureServiceAddress);
        var client = new CardPictureService.CardPictureServiceClient(channel);
    ...
    private async Task<CardListItem> ToCardListItemAsync(CardEntry entry, CancellationToken cancellationToken)
    {
        return new CardListItem
        {
            ...
            ImageUrl = await GetDownloadUrlAsync(entry.PhotoId, cancellationToken),
