...
/// loopback port, backed by a temp directory of catalog JSON files, so tests can point the BFF's
/// gRPC-calling logic at a real gRPC endpoint without a separately-run process.
/// </summary>
public sealed class CatalogServiceTestHost : IAsyncDisposable
{
    private readonly string catalogDirectory = Path.Combine(
        Path.GetTempPath(),
        $"NinjagoScannerBffTests_Catalog_{Guid.NewGuid():N}");
