...
- Fill `series_0_spinner.json` with the real Series 0 cards (221, from the Ninjago Spinners card list): Wave 1 (2011) as "Deck 1" categories with plain numbers 1-81 plus limited editions `LE1`-`LE9`, and Wave 2 (2012) as "Deck 2" categories. Wave 2 also numbers from 1, so its card numbers carry the prefix `WB` (`WB1`-`WB125`, limited editions `WBLE1`-`WBLE6`) to stay unique within the series; the prefix is letters only because the loader strips punctuation and sorts letter-prefix + digits.
