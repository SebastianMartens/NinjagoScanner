...
- The Overview page offers a layout switch between a tile/card grid and a dense table for the series summary; the choice is session-only (resets on reload), matching how `/collection`'s own view toggles already behave.
...
- `web-overview`: adds the per-series summary (counts, tile/table layout switch, unknown-series bucket) and the click-through navigation to `/collection` with the series pre-selected.
...
- `NinjagoScanner.Web/Components/Pages/Overview.razor`: new series summary UI (tile grid + table layout), layout switch, click-through links.
